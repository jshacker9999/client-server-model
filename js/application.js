...
NIL.Description = function (container) {
  ...
  function update(heading, content) {
    d3Container.style("opacity", 0).selectAll("text").remove();
    ...
    if (width > 550) {
      ...
      d3Container.selectAll("text").remove();
      d3Container.append("svg:text").text(firstLine);
      d3Container.append("svg:text").attr("y", 40).text(secondLine);
...
NIL.Diagram = function (config) {
  ...
  function merge(graph, state) {
    ...
    if (actions.length === 0) {
      throw (
        "The diagramState '" + state.diagramState + "' has 0 action statements."
      );
    }
    ...
    actions.forEach(function (action) {
      ...
      switch (action.method) {
        ...
        case "update":
          ...
          break;
        case "remove":
          var names = action.items.map(function (item) {
            return item.id;
          });
          graph.drop(names);
          break;
...
NIL.Display = (function () {
  ...
  function nodes(svgContainer, _nodes) {
    // Update the nodes
    var node = svgContainer.selectAll("g.node").data(_nodes, function (d) {
      return d._id;
    });
  ...
  function nodeOverlays(svgContainer, graph) {
    ...
    types.forEach(function (type) {
      nodes
        .data(graph.metaItems(type), function (d) {
          return d._id;
        })
        .call(NIL.Style[type]);
    });
  ...
  function livePaths(svgContainer, graph, namespace, reverse) {
    ...
    var pathData = connectionLinks(svgContainer, linkData, namespace).call(
      NIL.Style.pulsePath
    );
  ...
  function updateFlowIcons(svgContainer, linkData, paths, namespace, reverse) {
    ...
    paths.map(function (d, i) {
      if (d) {
        var slope =
          (linkData[i].target.y - linkData[i].source.y) /
    ...
    var markers = svgContainer
      .selectAll("g." + namespace)
      .data(markerData, function (d) {
        return d._id;
      });

    var markersEnter = markers
      .enter()
      .append("svg:g")
...
NIL.Navigation = function (config) {
  ...
  var current = 0,
    tableOfContents = new NIL.TableOfContents(
      config.tocSelector,
      config.stepToggleSelector
    );
  config.diagram.on("loaded", function () {
    config.diagram.courseSteps(function (steps) {
      draw();

      tableOfContents.updateList(steps).on("click", function (d, i) {
  ...
  function draw() {
    ...
    d3C
      ...
      .attr(
        "d",
        "M2.679,18.436c0,0.86,0.609,1.212,1.354,0.782l14.612-8.437c0.745-0.43,0.745-1.134,0-1.563L4.033,0.782   c-0.745-0.43-1.354-0.078-1.354,0.782V18.436z"
      );
...
NIL.Style = {
  ...
  clicker: function (nodes) {
    nodes.append("circle").attr("class", "clicker").attr("r", 50);
    return nodes;
  },
  icon: function (nodes) {
    nodes.append("circle").attr("class", "icon-bg").attr("r", 14);

    nodes
      .append("g")
      .append("use")
      .attr("xlink:href", function (d) {
        return d.iconsUrl + "#" + d.icon;
      })
      .attr("class", function (d) {
        return "icon " + d.icon;
      })
  ...
  crossOut: function (nodes) {
    ...
    nodes.exit().selectAll("use.cross-out").remove();
...
NIL.TableOfContents = function (containerSelector, stepToggleSelector) {
  ...
  function updateStep(index, total) {
    var current = index + 1;
    var menu =
      '<svg viewBox="0 0 90 90" enable-background="new 0 0 90 90" xml:space="preserve">' +
